...
export function Sidebar({ navItems = [], onLogout, isLoggingOut = false, children }: SidebarProps) {
    ...
    return (
        <aside 
            className={`min-h-screen flex flex-col px-4 py-6 gap-y-6 overflow-hidden bg-card border-r border-default-border
            transition-[width] duration-500 ease-in-out ${isExpanded ? "w-64" : "w-20"}`}
        >
